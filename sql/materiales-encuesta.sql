-- El material de una encuesta: presentaciones, hojas de cálculo, PDF
-- ------------------------------------------------------------------
...
-- presentación que se dio, el formato en Excel o el procedimiento en PDF. Hasta
-- ahora eso viajaba por WhatsApp y no quedaba pegado a la encuesta, así que
-- quien la abría un mes después no tenía de dónde sacarlo.
--
...
comment on table public.materiales_encuesta is
    'El material de apoyo de una encuesta: presentaciones, hojas de cálculo y PDF que se leen antes de contestarla.';
...
comment on column public.materiales_encuesta.archivo is
    'La ruta dentro del bucket materiales-evaluaciones. Es lo que hace falta para borrar el archivo.';
comment on column public.materiales_encuesta.url is
    'El publicUrl del archivo, que es lo que abre el enlace.';
...
-- retirar una versión vieja de la presentación.
